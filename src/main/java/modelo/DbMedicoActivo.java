...
public class DbMedicoActivo extends Conexion {
    ...
    public boolean guardar(MedicoActivo ma) {
        ...
        String sql = "INSERT INTO Medico (numero_cedula_med, nombre,fecha_contratacion,"
    ...
    public boolean eliminar(MedicoActivo ma) {
        PreparedStatement ps;
        Connection con = getConexion();
        String sql = "DELETE FROM Medico WHERE numero_cedula_med = ?";
        try {
            ps = con.prepareStatement(sql);
            ps.setString(1, ma.getCedula());
            ps.execute();
    ...
    public boolean buscar(MedicoActivo ma) {
        ...
        ResultSet rs;
        Connection con = getConexion();
        String sql = "SELECT * FROM Medico WHERE numero_cedula_med = ?";
        try {
            ps = con.prepareStatement(sql);
            ps.setString(1, ma.getCedula());
            rs = ps.executeQuery();
